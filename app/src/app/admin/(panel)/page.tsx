...
import type { CaseRow, PartyRow, ProductRow, TenantRow } from "@/lib/types";
...
export default async function AdminDashboard({
  ...
  const { tenant: tenantFilter, status: statusFilter } = await searchParams;
  const supa = db();

  const { data: tenants } = await supa.from("tenants").select("*").order("slug");
  ...
  let query = supa
    ...
    .limit(100);
  if (tenantFilter) {
    const t = tenantRows.find((t) => t.slug === tenantFilter);
  ...
  const [{ data: parties }, { data: products }] = await Promise.all([
    partyIds.length
      ? supa.from("parties").select("*").in("id", partyIds)
      : Promise.resolve({ data: [] }),
    productIds.length
      ? supa.from("products").select("*").in("id", productIds)
      : Promise.resolve({ data: [] }),
  ]);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex gap-2 text-sm flex-wrap">
        <Link href="/admin" className={chip(!tenantFilter && !statusFilter)}>
          Todos
        </Link>
        {tenantRows.map((t) => (
          <Link key={t.slug} href={`/admin?tenant=${t.slug}`} className={chip(tenantFilter === t.slug)}>
            {getTenant(t.slug).countryName}
          </Link>
        ))}
        {["submitted", "in_progress", "approved", "blocked"].map((s) => (
          <Link key={s} href={`/admin?status=${s}`} className={chip(statusFilter === s)}>
            ...
            {caseRows.length === 0 && (
              <tr>
                <td colSpan={7} className="px-4 py-12 text-center text-slate-brand-400">
                  Sin casos todavía. Creá el primero con “+ Nuevo cliente”.
                </td>
              </tr>
            )}
            {caseRows.map((c) => {
              ...
              return (
                <tr key={c.id} className="border-b border-slate-brand-100 last:border-0 hover:bg-forest-50/40 transition">
                  <td className="px-5 py-3.5">
                    <Link href={`/admin/cases/${c.id}`} className="font-semibold text-forest-800 hover:text-forest-950 hover:underline">
                  ...
                  <td className="px-4 py-3.5 text-slate-brand-700">{tenant ? getTenant(tenant.slug).countryName : "—"}</td>
